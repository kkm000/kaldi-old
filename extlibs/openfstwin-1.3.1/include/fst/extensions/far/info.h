...
void FarInfo(const vector<string> &filenames, const string &begin_key,
  ...
  if (!infos) {
    ...
    for (size_t i = 0; i < infos->size(); ++i) {
      ...
      if (ceil(log10((double)info.nstate)) + 2 > wnstate) //ChangedPD
        wnstate = ceil(log10((double)info.nstate)) + 2; //ChangedPD
      if (ceil(log10((double)info.narc)) + 2 > wnarc) //ChangedPD
        wnarc = ceil(log10((double)info.narc)) + 2; //ChangedPD
    }
